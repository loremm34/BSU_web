...
public class WelcomeFilter implements Filter{
    ...
    private void setUserInSession(HttpServletRequest request, HttpServletResponse response) {
        var user = authenticateService.validateAndGetAuthUser(request, response);
        request.getSession(true).setAttribute("user", user);
    }

    private void manageCookies(HttpServletRequest request, HttpServletResponse response, HttpSession session) {
    ...
    public boolean process(HttpServletRequest request, HttpServletResponse response) throws ServletException {
        try {
            setUserInSession(request,response);

            HttpSession session = request.getSession(true);
            manageCookies(request, response, session);
